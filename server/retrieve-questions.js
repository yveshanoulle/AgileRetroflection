...
function loadSheet(worksheetId, idCol, questionCol, authorCol, dateCol, callback) {
  ...
  return Spreadsheet.load({
    ...
    return spreadsheet.receive({getValues: true}, (err1, rowsObject) => {
...
module.exports = () => {
  async.parallel(
    ...
    (err, questions) => {
      ...
      return fs.writeFile('public/questions.json', stringifiedQuestions, err1 => {
        /* eslint no-console: 0 */
        if (err1) { return console.log(err1); }
        twitteravatar(twitterAPI, stringifiedQuestions).retrieveImageURLs((err2, urls) => {
          if (err2) { return console.log(err2); }
          return fs.writeFile('public/twitterImages.json', JSON.stringify(urls), err3 => {
            if (err3) { console.log(err3); }
          });
        });
        return console.log('questions updated');
      });
    }
  );
};
